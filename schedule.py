...
def call_chatbot(messages: List[BaseMessage]) -> str:
    ...
    try:
        ...
        extracted_name = extract_name(last_message)
        if extracted_name:
            query = f"""
            SELECT r.start_time, r.end_time, u.name as trainer_name
            FROM reservations r
            JOIN users u ON r.trainer_id = u.user_id
            WHERE r.user_id = (SELECT user_id FROM users WHERE name = '{extracted_name}')
            ORDER BY r.start_time;
            """
            result = run_query(query)
            if result and result != "데이터가 없습니다.":
                formatted_result = format_schedule_result(result)
