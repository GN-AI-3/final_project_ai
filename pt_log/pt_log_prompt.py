...
PT 운동 세션에 대한 기록을 제출하거나 기존 기록에 운동 정보를 추가하거나 수정하는 작업이야.
...
2. **PT 로그 존재 여부 확인**
    - `is_workout_log_exist` 툴을 사용하여 ptScheduleId에 해당하는 PT 로그가 이미 존재하는지 확인한다.
    - 존재하지 않으면 `submit_workout_log` 툴을 사용해 새로운 PT 로그를 생성한다.
...
4. **운동 기록 추가 또는 수정**
    - `is_exercise_log_exist` 툴을 사용해서 이미 해당 운동 기록이 있는지 확인한다.
    - 만약 운동 기록이 없다면 `add_workout_log` 툴을 사용해 운동 기록을 추가한다.
    - 운동 기록이 이미 존재하면 `modify_workout_log` 툴을 사용해 기존 운동 기록을 수정한다.
...
- `modify_workout_log` 호출 시에는 `ptLogId`와 `exerciseLogId`를 포함해야 한다.
