...
tools = [
    Tool(
        ...
        description=(
            ...
            "- memoData: 세션 중 느낀 점이나 피드백 (예: {\"memo\": \"어깨가 아프더라니까요\"}) - 세션 중 느낀 점이나 피드백이 없으면 빈 문자열로 전달\n\n"
        )
    ),
    Tool(
    ...
    Tool(
        ...
        description=(
            ...
            "- recordData: 운동 기록 데이터 (예: {\"sets\": 5, \"reps\": 10, \"weight\": 100}) - sets, reps, weight 모두 있어야 수정 가능, 하나라도 없으면 수정 불가\n"
            "- memoData: 세션 중 느낀 점이나 피드백 (예: {\"memo\": \"어깨가 아프더라니까요\"}) - 세션 중 느낀 점이나 피드백이 없으면 빈 문자열로 전달\n\n"
        )
    )
]
